...
def download_thread(url, start, end, filename, thread_num):
    print(start, end)
    with requests.get(url=url, stream=True, headers={
            'Range': 'bytes=%s-%s' % (start, end)}) as r:
...
def download_manager(url, threads, filename=None):
    ...
    for thread_num, down_range in zip(range(10), ranges):
        t = threading.Thread(target=download_thread, args=(
            url, down_range[0], down_range[1], filename, thread_num))
        threads.append(t)
        t.start()
...
if __name__ == '__main__':
    filename = 'a.txt'
    threads = 10
    url = 'https://raw.githubusercontent.com/party98/open-event-server/\
